...
class MyhexunsqdSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        option = Options()
        option.add_argument('--headless')  # 设置为Headless
        option.add_argument('--disable-gpu')
        option.binary_location = r'C:\Program Files (x86)\Google\Chrome\Application\chrome.exe'  # chrome 安装路径
        self.browser = webdriver.Chrome(chrome_options=option)

        # self.brower = webdriver.Chrome(executable_path=r"C:\scrapy\resource\chromedriver.exe")
        super(MyhexunsqdSpider, self).__init__()
    ...
    def parse(self, response):
       ...
       #思路还是采用锚点的方式先找到锚点，然后逐个item进行赋值 ；如果锚点不明确，则可以使用下面的方式直接利用XPATH绝对路径来查找
        for i in range(num):
            ...
            item["hit"]=response.xpath('//div[@class="ArticleInfo"]/a/span/text()').extract()[i]
            item["comment"]=response.xpath('//div[@class="ArticleInfo"]/span/text()').extract()[i]
